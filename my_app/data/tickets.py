...
def update_ticket_status(conn, ticket_id, new_status):
    ...
    if len(locals()) == 3 and ticket_id is not None and new_status is not None and isinstance(conn, (str,)) is False:
        # called as update_ticket_status(conn, ticket_id, new_status)
        conn_obj, ticket_id_val, new_status_val = conn, ticket_id, new_status
        conn = conn_obj
        managed = False
    else:
        # called as update_ticket_status(ticket_id, new_status)
        ticket_id_val, new_status_val = conn, ticket_id
        conn = connect_database()
        managed = True
    ...
    try:
        cursor.execute(sql, (new_status_val, ticket_id_val))
        conn.commit()
...
def delete_ticket(conn, ticket_id=None):
    """Delete ticket by id. Supports both delete_ticket(ticket_id) and delete_ticket(conn, ticket_id)."""
